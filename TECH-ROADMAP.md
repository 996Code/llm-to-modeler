...
> |-----------|--------------------------------------------------------|
> | env `ASSET_BASE_URL`（数据操作独立地址） | `submit_data`/`query_data` 按 `service_name` 寻址，与配置类操作同一套解析 |
...
> - **知识图谱插件**：导入流水线（LLM 批量抽取/断点续跑/熔断/**实体消歧**）+
>   混合检索问答 + G6 图谱浏览管理页。消歧=词表播种（图内存量实体+别名
>   注入抽取词表）+ 收尾对账（按"名字∈对方aliases+同类型"的文本指认
>   并查集合并,canonical 取最早入库名,关系边真迁移）——同一人物跨块
>   不同写法（张小凡/鬼厉）不再裂成多节点,零启发式猜测
...
2. [系统架构](#2-系统架构)
3. [技术栈](#3-技术栈)
4. [核心引擎：LangGraph StateGraph](#4-核心引擎langgraph-stategraph)
5. [插件自动发现机制](#5-插件自动发现机制)
6. [数据流全链路](#6-数据流全链路)
7. [API 设计](#7-api-设计)
8. [MCP 协议层](#8-mcp-协议层)
...
### 10.4 模式对比【历史形态——当前为 iframe 悬浮窗 + 信封协议】
...
### 10.6 embed.js SDK（给主系统引入）【历史方案——现为 hostPort 信封契约】
