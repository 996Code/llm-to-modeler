...
| 长文档导入 | 全局 LLM 限速（RPM/TPM 双令牌桶 + 429 全局退避）;`extraction_model` 设置可单独给导入换更快的模型（对话/检索链路不受影响）;失败自动续跑（非欠费/鉴权类,5 分钟退避）;滑窗 ETA 预估 |
...
| 调用日志 | `call_logs` 审计流水（LLM/上游/**图谱/向量检索**），按会话/类型过滤，检索类型带召回量/匹配度列，抽屉查看请求/响应全文 |
...
| **图谱/向量检索**（种子词与逐词命中、子图召回量与三元组明细、向量逐条相似度分数） | `call_logs`（call_type=graph/vector；检索摘要行直接可读，明细展开可查） |
