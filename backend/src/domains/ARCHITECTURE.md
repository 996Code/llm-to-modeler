...
"第X章/序章/尾声"等与 markdown `#` 同等对待),章节头起新块且不加
前块尾部 overlap——长小说不会出现"章标题在块中间"的跨章污染
(诛仙全本实测 1661 块 0 跨章)。

**经验值**(156 万字小说,单次抽取 150~400s 的慢模型):串行约 70 小时;
换快模型 + 并行 8 后约 2~4 小时。瓶颈排序:抽取模型速度 > 并行度 > 限速配额。
