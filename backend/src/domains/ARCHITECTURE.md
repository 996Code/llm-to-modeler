...
百万字级文档(整本小说/大型手册)导入的完整支撑链,全部在设置页可调
(插件设置 →「抽取与批次」),不需要改代码或重启:
...
"第X章/序章/尾声"等与 markdown `#` 同等对待),章节头起新块且整章
入单块(上限 `chunk_structural_max_chars`,默认 10000 字,超长才二切),
目录合并为单块——长小说不会出现"章标题在块中间"的跨章污染
(诛仙全本实测 276 块,目录 2 块 + 每章 1 块,0 跨章)。

**经验值**(诛仙 156 万字/276 块实测):qwen3.7-plus + 并行 2 + 批 4,
全程 6.7 小时(词表带别名后单块 LLM 耗时 60~110s);瓶颈排序:
抽取模型速度 > 并行度 > 限速配额。给导入换快模型(`extraction_model`)
是第一优化项。
