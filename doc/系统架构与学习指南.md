...
flowchart TD
    ...
    subgraph Classify["classify_intent 节点 nodes.py:54"]
        ...
        C2["_build_intent_prompt() 动态拼接工具列表<br/>生成 system prompt（不依赖 Jinja2）"]
        C3["llm_client.chat_json() → LLM 分类<br/>返回 {tools: [tool_name], reason: ...}"]
        C4{"安全检查：<br/>工具是否需要已有制品？"}
    end

    Classify -->|写入 state.tool_name| Router["LangGraph 调度器<br/>读取 route_by_tool(state)"]
    Router -->|tool_name 非空 → 返回 tool| Execute["→ execute_tool 节点（阶段二）"]
    Router -->|tool_name 为空 → 返回 end| End["→ END（兜底走 chat 工具）"]
    C4 -->|需要制品但不存在| Skip["跳过该工具，继续遍历候选"]
```
...
```mermaid
flowchart TD
    Entry["route_by_tool 返回 'tool'"] --> ExecuteNode

    subgraph ExecuteNode["execute_tool 节点 nodes.py:140"]
        E1["registry.get(tool_name) 取工具实例"]
        E2["构建 ToolContext（注入 llm_client/asset_client/emit/forward_headers）"]
        E3["调用 tool.execute(tool_state, ctx)"]
        E4{"检查 ToolResult"}
    end

    E4 -->|artifact/reply/error| ToHandle["写入 state.tool_result<br/>→ 无条件边到 handle_result（阶段三）"]
    E4 -->|ask 追问| Interrupt["调 interrupt(questions) 挂起图<br/>LangGraph 自动保存 Checkpoint"]

    Interrupt --> Check["stream_graph 检测 graph.get_state().tasks[].interrupts"]
    Check --> SSE["推 SSE needsClarification → 前端渲染追问卡片"]
    SSE --> User["用户回答（新 HTTP 请求，带 answers）"]
    User --> Resume["stream_graph 走 Command(resume=answers)"]
    Resume --> Inject["execute_tool_node 从断点恢复<br/>answers 注入 tool_state['clarify_answers']<br/>清除 _need_clarify 标记"]
    Inject --> Rerun["tool.execute 重跑<br/>（这次有完整信息，不再追问）"]
```
...
```mermaid
flowchart LR
    Start["execute() 调 run_pipeline()"] --> Emit["先 emit pipeline_definition<br/>前端显示 6 步进度条"]
    Emit --> S1

    subgraph S1["1. _step_fetch_guide"]
        S1a["ctx.asset_client.get_guide()<br/>→ HttpAssetClient → UpstreamClient<br/>→ GET 上游 :7001/guide.json<br/>→ sanitize_obj 清洗"]
    end

    subgraph S2["2. _step_list_assets"]
        S2a["ctx.asset_client.list_templates()<br/>→ 获取可用模板列表"]
    end

    subgraph S3["3. _step_parse_fields"]
        S3a["ctx.llm_client.chat_json()<br/>LLM 解析自然语言→字段列表"]
        S3b{"信息不足？"}
    end

    subgraph S4["4. _step_fetch_templates"]
        S4a["根据字段类型匹配模板<br/>ctx.asset_client.get_template()"]
    end

    subgraph S5["5. _step_generate"]
        S5a["ctx.llm_client.chat_json()<br/>LLM 生成表单配置 JSON"]
    end

    subgraph S6["6. _step_validate"]
        S6a["ctx.asset_client.validate_artifact()<br/>→ 上游校验"]
        S6b{"校验结果"}
    end

    S1 --> S2 --> S3
    S3b -->|否| S4 --> S5 --> S6
    S3b -->|是| Clarify["raise ClarificationRaised<br/>→ ToolResult(ask)<br/>→ interrupt 追问"]
    S6b -->|有 errors| Retry["retry_count < 3 ?<br/>重跑 _step_generate（带错误上下文）"]
    Retry --> S5
    S6b -->|通过 / 只有 warnings| Done["返回 ToolResult(artifact=配置JSON)<br/>→ 无条件边到 handle_result"]
```
...
```mermaid
flowchart TD
    Entry["execute_tool 无条件边进入"] --> Result

    subgraph Result["handle_result 节点 nodes.py:282"]
        R1{"ToolResult 状态分流（按优先级）"}
    end

    R1 -->|error_for_llm 非空| SSE1["SSE result type=error<br/>错误信息回流"]
    R1 -->|reply 非空| SSE2["SSE result type=general<br/>纯文本回复（闲聊）"]
    R1 -->|artifact + config| SSE3["SSE result type=config<br/>配置卡（含 config + valid + validationErrors）"]
    R1 -->|artifact + data| SSE4["SSE result type=data<br/>数据卡（含 data + summary）"]
    R1 -->|都没有| SSE5["SSE error<br/>兜底错误"]

    SSE3 --> Router2["route_after_result(state)"]
    SSE4 --> Router2
    Router2 -->|"tool_result 有值 → done"| End["→ END"]
    Router2 -->|"tool_result 为 None → rerun"| Back["→ 回到 execute_tool 重跑"]
```
...
```mermaid
stateDiagram-v2
    [*] --> classify_intent: add_edge(START, classify_intent)

    classify_intent --> execute_tool: 条件边 route_by_tool 返回 "tool"
    classify_intent --> [*]: 条件边 route_by_tool 返回 "end"

    execute_tool --> handle_result: 无条件边 add_edge(execute_tool, handle_result)
    note right of execute_tool
        interrupt() 在节点内部调用
        不是边——挂起的是节点本身
        Checkpoint 自动保存 state
        用户回答后 Command(resume)
        从 interrupt() 位置恢复执行
    end note

    handle_result --> execute_tool: 条件边 route_after_result 返回 "rerun" (追问恢复后 tool_result=None)
    handle_result --> [*]: 条件边 route_after_result 返回 "done"
```
...
```python
# nodes.py:246-250 — interrupt 触发（在 execute_tool_node 内部，不是边）
# 这是整个追问机制的核心：interrupt() 会抛 PauseSignal 挂起当前执行
answer = interrupt(interrupt_value)  # ★ 挂起，等 Command(resume=answers)

# nodes.py:256-263 — resume 后从 interrupt 位置恢复，注入答案 + 清标记
