...
```mermaid
flowchart TD
    Chat["POST /api/config/chat<br/>config.py:166"] --> StreamGraph
    StreamGraph["stream_graph()<br/>stream.py:46"] -->|"线程池 run_in_executor"| RunGraph["graph.stream()<br/>stream.py:204"]
    RunGraph --> Classify["classify_intent_node<br/>→ llm_client.chat_json()<br/>nodes.py:97"]
    Classify -->|"route_by_tool 返回 tool"| Execute["execute_tool_node<br/>→ tool.execute(state, ctx)<br/>nodes.py:220"]
    Execute -->|"无条件边"| Handle["handle_result_node<br/>→ 分流 ToolResult 三态<br/>nodes.py:282"]
    Handle -->|"route_after_result 返回 done"| EndNode["END"]
    Handle -->|"sse_events"| StreamGraph
    StreamGraph --> Store["ConversationStore SQLite"]
```
...
```mermaid
flowchart TD
    Execute["tool.execute(tool_state, ctx)"] --> Pipeline["run_pipeline(state, ctx)<br/>tool.py:168"]
    Pipeline -->|"先 emit"| Def["emit pipeline_definition<br/>前端显示进度条"]
    Pipeline --> S1["1._step_fetch_guide → asset_client"]
    S1 --> S2["2._step_list_assets → asset_client"]
    S2 --> S3["3._step_parse_fields → llm_client"]
    S3 -->|"信息不足"| AskPath["→ ToolResult(ask)<br/>→ interrupt 追问"]
    S3 -->|"正常"| S4["4._step_fetch_templates"]
    S4 --> S5["5._step_generate → llm_client"]
    S5 --> S6["6._step_validate → asset_client"]
    S6 -->|"有 errors, retry<3"| S5
    S6 -->|"通过"| Result["ToolResult(artifact)"]
```
...
#### 8.3 完整时序图（以"创建请假表单"为例）

```mermaid
sequenceDiagram
    participant U as 用户
    participant F as 前端
    participant S as stream_graph
    participant G as LangGraph
    participant N as execute_tool_node
    participant T as CreateFormTool
    participant CP as Checkpoint

    U->>F: "创建请假表单"
    F->>S: POST /api/config/chat
    S->>G: graph.stream(input_data)
    G->>N: 调度 execute_tool_node
    N->>T: tool.execute()
    T->>T: _step_parse_fields
    T-->>N: ClarificationRaised(追问类型)
    N->>N: interrupt(questions)
    Note over N: ★ 抛 PauseSignal 挂起
    N->>CP: 自动保存状态
    N-->>G: 执行暂停
    G-->>S: stream 结束
    S->>S: 检测 get_state().tasks[].interrupts
    S-->>F: SSE needsClarification
    F-->>U: 渲染追问卡片

    U->>F: 选择"年假"
    F->>S: POST /api/config/chat {answers}
    S->>G: graph.stream(Command(resume=answers))
    G->>CP: 从 Checkpoint 恢复状态
    G->>N: 从 interrupt 位置继续
    Note over N: ★ interrupt() 返回 answers
    N->>N: 注入 clarify_answers
    N->>N: 清除 _need_clarify 标记
    N->>T: tool.execute() 重跑
    T->>T: _step_parse_fields（这次信息完整）
    T-->>N: ToolResult(artifact=配置JSON)
    N-->>G: 正常返回
    G-->>S: stream 继续
    S-->>F: SSE artifact
    F-->>U: 显示表单配置
