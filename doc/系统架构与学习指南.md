...
| 技术 | 用途 | 学习重点 |
|------|------|---------|
| **Python 3.12** | 后端语言 | async/await、TypedDict、Pydantic |
| **FastAPI** | Web 框架 | 路由、SSE、依赖注入 |
| **LangGraph** | 状态机编排 | StateGraph、node、conditional edge、interrupt/resume |
| **Pydantic** | 数据校验 | BaseModel、Field、ConfigDict |
| **Jinja2** | Prompt 模板 | 变量、条件、循环 |
| **httpx** | HTTP 客户端 | 异步请求 |
| **SQLite** | 本地数据库 | SQL 基础即可 |
| **Vue 3 + TS** | 前端 | Composition API、Pinia |
...
#### 必读
- 本文档「第二章 核心数据流」+「第三章 状态机」+「第七章 追问机制」
