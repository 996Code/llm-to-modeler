...
flowchart TD
    ...
    subgraph Execute["execute_tool 节点 nodes.py:140"]
...
flowchart TD
    ...
    subgraph Check["handle_result 节点 nodes.py:282"]
        R1{"ToolResult 状态分流"}
    end
...
| 边 | 函数 | 逻辑 |
|---|------|------|
| `route_by_tool` | `graph.py:111` | 选了工具 → execute_tool；没选 → END |
| `route_after_result` | `graph.py:73` / `nodes.py:383` | `tool_result is None` → rerun（追问恢复后重跑）；否则 → END |
...
| 步骤 | 调用方 → 被调用方 | 文件:行 | 说明 |
|------|-------------------|---------|------|
| 1 | 前端 → `POST /api/config/chat` | `config.py:166` | SSE 流式请求 |
| 2 | `chat()` → `stream_graph()` | `stream.py:46` | 构建 input，线程池跑图 |
...
| 15 | `stream_graph` → `ConversationStore.save()` | `stream.py:292` | 持久化对话 |
...
| 数据安全 | append-only | `services/conversation_store.py:572` | events 表只 INSERT 不 UPDATE |
| 数据安全 | 调用审计 | `services/conversation_store.py:454` | `save_call_log` 记录 LLM/上游调用 |
...
| 资产抽象 | `backend/src/sdk/asset_client.py:24` | `AssetClient` ABC |
