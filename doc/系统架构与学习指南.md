...
> **本阶段产出**：`state["selected_tool"]`（工具名）+ `state["tool_input"]`（工具参数）
> **下一阶段衔接**：`route_by_tool` 条件边判断——选了工具进入阶段二，没选工具直接结束。

```mermaid
flowchart TD
    Input["用户消息 + 压缩后的历史"] --> Classify

    subgraph Classify["classify_intent 节点 nodes.py:54"]
        C1["从 registry.all() 读取所有工具描述"]
        C2["渲染 intent.j2 prompt 模板<br/>（动态拼入工具列表）"]
        C3["LLM 分类 → 选工具 + 提取参数"]
        C4{"安全检查：<br/>工具是否需要已有制品？"}
    end

    Classify -->|选了工具| Route["route_by_tool → execute_tool"]
    Classify -->|未选工具/闲聊| End["直接回复 → END"]
    C4 -->|需要制品但不存在| Skip["跳过该工具 → 重新分类"]
```
...
> **本阶段职责**：执行选中的工具。如果是 `CompositeTool`（多步管线），顺序执行 `_step_*` 方法。
> **本阶段产出**：`ToolResult`（三态：artifact 制品 / reply 回复 / ask 追问）
> **追问机制**：工具返回 `ask` 时调 `interrupt()` 挂起图，等用户回答后 `Command(resume=answers)` 恢复。
> **上一阶段衔接**：由阶段一 `route_by_tool` 进入。

```mermaid
flowchart TD
    Tool["selected_tool"] --> Execute

    subgraph Execute["execute_tool 节点 nodes.py:128"]
        E1["创建 ToolContext（注入依赖）"]
        E2["调用 tool.execute(state, ctx)"]
        E3{"ToolResult 状态"}
    end

    E3 -->|artifact 制品| Handle["→ 阶段三 handle_result"]
    E3 -->|reply 回复| Handle
    E3 -->|ask 追问| Interrupt["interrupt() 挂起图"]
    E3 -->|error 错误| Handle

    Interrupt --> SSE["SSE needsClarification → 前端"]
    SSE --> User["用户回答"]
    User --> Resume["Command(resume=answers)"]
    Resume --> Rerun["route_after_result → rerun<br/>重跑 execute_tool"]
```
...
**以 CreateFormTool 为例的 6 步管线**：

```mermaid
flowchart LR
    S1["1.fetch_guide<br/>获取填写指南"] --> S2["2.list_assets<br/>列可用模板"]
    S2 --> S3["3.parse_fields<br/>LLM 解析字段"]
    S3 --> S4["4.fetch_templates<br/>取模板详情"]
    S4 --> S5["5.generate<br/>LLM 生成配置"]
    S5 --> S6["6.validate<br/>上游校验"]
    S6 -->|失败| S5
    S6 -->|成功| Done["ToolResult(artifact)"]
    S3 -->|信息不足| Clarify["ClarificationRaised<br/>→ interrupt 追问"]
```

> 每步通过 `ctx.emit("stage", ...)` 实时推送 SSE 事件，前端显示管线进度。`validate` 失败时内部 retry（重跑 `generate`，最多 3 次）。
...
```mermaid
flowchart TD
    Result["ToolResult"] --> Check

    subgraph Check["handle_result 节点 nodes.py:248"]
        R1{"ToolResult 状态分流"}
    end

    R1 -->|artifact + config| SSE1["SSE result<br/>type=config 配置卡"]
    R1 -->|artifact + data| SSE2["SSE result<br/>type=data 数据卡"]
    R1 -->|reply| SSE3["SSE result<br/>type=general 纯文本"]
    R1 -->|error| SSE4["SSE result<br/>type=error 错误信息"]
    R1 -->|None rerun| Loop["返回 execute_tool 重跑"]
```
...
stateDiagram-v2
    ...
    classify_intent --> execute_tool: route_by_tool (选了工具)
    classify_intent --> END_FINAL: route_by_tool (未选工具)

    execute_tool --> handle_result: 正常完成
    execute_tool --> INTERRUPTED: interrupt() 追问

    INTERRUPTED --> execute_tool: Command(resume=answers)

    handle_result --> END_FINAL: 有结果
...
> - **节点（node）**：状态机的一个执行步骤，是一个 Python 函数。`classify_intent_node(state) → state`。
> - **条件边（conditional edge）**：根据状态决定下一步去哪。`route_by_tool(state) → "execute_tool" | "END"`。类比 Spring State Machine 的 `Transition` + `Guard`。
> - **InMemorySaver**：LangGraph 的内存检查点保存器。每次节点执行后自动保存状态，支持 interrupt/resume。类比 JPA 的 `@Version` 乐观锁 + 持久化，但是自动的。

**状态机三节点**（`engine/graph.py:29`）：

| 节点 | 文件:行 | 职责 |
|------|---------|------|
| `classify_intent` | `nodes.py:54` | LLM 意图识别，从 registry 选工具 |
| `execute_tool` | `nodes.py:128` | 执行工具，支持 interrupt/resume 追问 |
| `handle_result` | `nodes.py:248` | 处理 ToolResult 三态分流 |

**两条条件边**：

| 边 | 函数 | 逻辑 |
|---|------|------|
| `route_by_tool` | `graph.py:67` | 选了工具 → execute_tool；没选 → END |
| `route_after_result` | `graph.py:73` / `nodes.py:336` | `tool_result is None` → rerun（追问恢复后重跑）；否则 → END |
...
| 步骤 | 调用方 → 被调用方 | 文件:行 | 说明 |
|------|-------------------|---------|------|
| 1 | 前端 → `POST /api/config/chat` | `config.py:105` | SSE 流式请求 |
| 2 | `chat()` → `stream_graph()` | `stream.py:25` | 构建 input，线程池跑图 |
| 3 | `stream_graph` → `graph.stream()` | LangGraph | 在线程池跑同步 stream |
| 4 | `classify_intent_node` → `llm_client.chat_json()` | `nodes.py:92` | LLM 选工具 |
| 5 | `classify_intent_node` → `registry.get("create_form")` | `nodes.py:98` | 取工具实例 |
| 6 | `execute_tool_node` → `tool.execute(state, ctx)` | `nodes.py:196` | 执行工具 |
| 7 | `CreateFormTool.run_pipeline` → `_step_fetch_guide()` | `create_form.py:113` | 获取填写指南 |
| 8 | `_step_fetch_guide` → `ctx.asset_client.get_guide()` | `asset_client` | 调上游 API |
| 9 | `HttpAssetClient.get_guide` → `UpstreamClient.get_guide()` | `http_asset_client.py:61` | HTTP 请求 :7001 |
| 10 | `HttpAssetClient` → `sanitize_obj()` | `http_asset_client.py:44` | 清洗返回数据 |
| 11 | `_step_parse_fields` → `ctx.llm_client.chat_json()` | `create_form.py:147` | LLM 解析字段 |
| 12 | `_step_validate` → `ctx.asset_client.validate_artifact()` | `create_form.py:254` | 上游校验 |
| 13 | 校验失败 → `_step_generate()` retry | `create_form.py:283` | 重跑生成（最多 3 次） |
| 14 | `handle_result_node` → SSE result 事件 | `nodes.py:282` | 推送制品 |
| 15 | `stream_graph` → `ConversationStore.save()` | `stream.py:206` | 持久化对话 |

> **📖 深入阅读**：每个步骤的源码实现见「第十五章 关键实现深度解析」。工具执行的 6 步管线详见「第十五章 15.7」。
...
graph TB
    ...
    subgraph Engine["Engine 层 — 零领域知识"]
    ...
    subgraph Domains["Domain Pack 层 — 领域知识"]
    ...
    subgraph Infra["基础设施"]
        ...
        Store["services/conversation_store.py<br/>SQLite 持久化"]
        Upstream["services/upstream_client.py<br/>上游 HTTP 客户端"]
        Adapter["adapters/http_asset_client.py<br/>HTTP 适配器"]
    end
...
flowchart TD
    ...
    Wait --> Answer["用户回答"]
    Answer --> Post["POST /api/config/chat {answers}"]
...
graph TB
    subgraph Security["安全多层次防御"]
        ...
        subgraph L2["2. 工具安全"]
            direction LR
            L2a["Fail-Closed 默认值<br/>is_destructive=True / is_read_only=False"]
            L2b["requires_existing_artifact<br/>无制品时跳过工具（防 LLM 选错）"]
            L2c["validate_input 钩子<br/>execute 前语义校验"]
        end
...
| 层次 | 措施 | 代码位置 | 说明 |
|------|------|---------|------|
| 输入安全 | Unicode 清洗 | `sdk/sanitize.py:24` | NFKC 归一化 + 删零宽/方向反转/BOM/私用区字符 |
...
| 工具安全 | 输入校验钩子 | `sdk/tool.py:117` | `validate_input` 在 execute 前调 |
| 传输安全 | 请求头白名单 | `api/config.py:69` | 只转发匹配前缀的头 |
| 传输安全 | 排除 hop-by-hop | `api/config.py:77` | host/content-length 等不转发 |
| 日志安全 | 凭证脱敏 | `engine/logging_filter.py:50` | 正则替换 Bearer/sk-/Authorization/Cookie |
| 日志安全 | 挂 root logger | `main.py:42` | `install_redact_filter()` 启动时挂载 |
| 数据安全 | append-only | `services/conversation_store.py:413` | events 表只 INSERT 不 UPDATE |
| 数据安全 | 调用审计 | `services/conversation_store.py:335` | `save_call_log` 记录 LLM/上游调用 |
...
| 模块 | 文件 | 关键入口 |
|------|------|---------|
| 应用入口 | `backend/src/main.py:45` | `lifespan()` — 启动流程 |
| FastAPI 应用 | `backend/src/main.py:101` | `app = FastAPI(...)` |
| 状态机构建 | `backend/src/engine/graph.py:29` | `build_graph()` — 三节点 + 两条件边 |
| 统一对话入口 | `backend/src/api/config.py:105` | `chat()` — SSE 流式 |
...
| 工具执行节点 | `backend/src/engine/nodes.py:128` | `execute_tool_node()` — 执行 + interrupt/resume |
| 结果处理节点 | `backend/src/engine/nodes.py:248` | `handle_result_node()` — ToolResult 三态分流 |
| 路由函数 | `backend/src/engine/nodes.py:336` | `route_after_result()` — rerun 判断 |
| SSE 桥接 | `backend/src/engine/stream.py:25` | `stream_graph()` — graph.stream → SSE |
| 上下文压缩 | `backend/src/engine/compression.py:141` | `CompressionSidechain` — 后台压缩+熔断器 |
...
| 工具注册表 | `backend/src/sdk/registry.py:9` | `ToolRegistry` — register/all/get |
| 资产抽象 | `backend/src/sdk/asset_client.py:19` | `AssetClient` ABC |
| Unicode 清洗 | `backend/src/sdk/sanitize.py:24` | `sanitize_text()` — NFKC + 危险字符移除 |

#### Domain Packs

| 模块 | 文件 | 关键入口 |
|------|------|---------|
| 自动发现 | `backend/src/domains/__init__.py:89` | `load_all_packs()` — 扫描 + 注册 |
| 表单创建工具 | `backend/src/domains/njmind_form/tools/create_form.py:24` | `CreateFormTool` — 6 步管线 |
| 表单修改工具 | `backend/src/domains/njmind_form/tools/modify_form.py:22` | `ModifyFormTool` — 3 步管线 |
| 闲聊工具 | `backend/src/domains/njmind_form/tools/chat.py:35` | `ChatTool` — 动态能力上报 |
| 表单模型 | `backend/src/domains/njmind_form/models.py:20` | `FormConfig` — 表单配置 Pydantic 模型 |
...
| SSE 管理 | `backend/src/api/sse.py:33` | `StreamManager` — 线程安全队列 |
...
#### 动手练习
1. 设置断点在 `nodes.py:128` 的 `execute_tool_node`，跟踪一次完整执行
...
**classify_intent_node 安全检查**（行 96-105）：
...
- 兜底（行 110-112）：没选到工具 → `_get_fallback_tool_name()`（优先级：chat → 安全只读 → 第一个非 artifact 工具）
- 失败处理：LLM 调用异常只 warning 不崩

**execute_tool_node 的 interrupt/resume 核心**（行 204-239）：

```python
# nodes.py:212-219 — interrupt 触发
interrupt_value = {"questions": questions_data, "summary": questions_text}
answer = interrupt(interrupt_value)  # ★ 挂起，等 Command(resume=answers)

# nodes.py:222-230 — resume 后重跑前的关键清理
tool_state["clarify_answers"] = answer  # 注入答案
# ★ 关键：清除中断标记，否则 run_pipeline 会在第一步前 break
tool_state.pop("_need_clarify", None)
tool_state.pop("_clarify_spec", None)
```

> **WHY 必须清标记**（行 226-228 注释）：不清 `_need_clarify`，`run_pipeline`（`tool.py:170`）会在第一个 step 前就 `break`，追问答案永远不会被消费。这是 interrupt/resume 重入的关键坑。

**route_after_result rerun 判断**（行 336-342）：

```python
# tool_result is None 且有 tool_name → 返回 "rerun"（追问恢复后重跑）
...
# graph.py:82-84 — InMemorySaver checkpoint
checkpointer = InMemorySaver()
graph = workflow.compile(checkpointer=checkpointer)
```

- `thread_id = conversation_id`（见 `stream.py:80`），每个会话一个 checkpoint 线程
- InMemorySaver 状态存内存，重启丢失；生产可换 SqliteSaver/PostgresSaver

#### 13.3 SSE 桥接详解（`engine/stream.py`）

**线程池跑同步 graph.stream**（行 134-145）：
...
await loop.run_in_executor(None, _run_graph)  # 线程池跑同步循环
```

> **WHY**：`graph.stream` 是同步 API，但 SSE 产出要 async generator → 用线程池执行器跑同步循环，通过 `call_soon_threadsafe` 桥接到事件循环。

**Command(resume=answers) 逻辑**（行 60-77）：

```python
# answers 非空 → Command(resume)，从断点继续；为空 → 全新 input
if answers:
...
### backend/src/engine/ — Engine 引擎层（8 个文件）

#### `engine/nodes.py` — LangGraph 节点函数
| 行号 | 定义 | 描述 |
|------|------|------|
| 35 | `configure()` | 模块级依赖注入（registry/llm_client/asset_client/conversation/prompt_loader），由 graph.py 构建时调用 |
| 54 | `classify_intent_node(state)` | LLM 意图识别节点：动态构建 prompt，从 registry 选工具 |
| 128 | `execute_tool_node(state)` | 执行选中工具：构建 ToolContext + emit 回调，遇到 ask 调 interrupt 挂起；resume 时注入 answers 重跑 |
| 248 | `handle_result_node(state)` | 处理工具结果三态分流（error/reply/ask/artifact），按 artifact_type 构造 SSE 事件 |
| 324 | `route_by_tool(state)` | 条件边：classify_intent 后按 tool_name 路由 |
| 336 | `route_after_result(state)` | 条件边：tool_result 为空且需重跑→rerun，否则→done |
| 348 | `_build_intent_prompt(has_existing_config)` | 从 registry.all() 动态生成意图识别 prompt |
| 366 | `_get_fallback_tool_name()` | 兜底工具名（优先级：chat → 安全只读 → 第一个非 artifact 工具） |

#### `engine/graph.py` — StateGraph 构建
| 行号 | 定义 | 描述 |
|------|------|------|
| 29 | `build_graph(...)` | 构建+编译 StateGraph：configure 注入 → 3 节点 → 边/条件边 → compile(InMemorySaver) |

#### `engine/graph_state.py` — 图状态定义
| 行号 | 定义 | 描述 |
|------|------|------|
| 20 | `GraphState(TypedDict)` | 状态字段：输入/意图/执行/追问/sse_events |

#### `engine/stream.py` — SSE 桥接
| 行号 | 定义 | 描述 |
|------|------|------|
| 25 | `stream_graph(...)` | 线程池跑 graph.stream → SSE；处理 GraphInterrupt→needsClarification；answers 走 Command(resume) |
| 195 | `_save_conversation(...)` | 简单保存一轮对话（追问摘要场景） |
| 206 | `_save_result_conversation(...)` | 保存工具结果：写 user+assistant summary，更新会话配置/标题 |
...
| 66 | `build_compressed_history(...)` | 历史格式化为文本（保留最近 N 条+截断） |
| 88 | `should_compress(...)` | token 超有效窗口 70% → True |
| 101 | `CompressionCircuitBreaker` | 熔断器：record_failure/record_success/is_tripped |
| 141 | `CompressionSidechain` | 压缩 forked sidechain：compress_async 立即返回+后台压缩 |

#### `engine/conversation.py` — 多轮对话管理器
| 行号 | 定义 | 描述 |
|------|------|------|
| 15 | `ConversationManager` | append-only 事件流管理器 |
| 22 | `.append(conv_id, kind, payload)` | 追加事件 |
| 32 | `.load(conv_id)` | 加载重建会话状态 |
| 73 | `.save(conv_id, user_input, result)` | 保存一轮对话 |
| 111 | `.save_pending_ask(...)` | 保存追问现场 |
| 121 | `.load_pending_ask(conv_id)` | 加载追问现场 |

#### `engine/prompt_loader.py` — Prompt 装配器
| 行号 | 定义 | 描述 |
|------|------|------|
| 29 | `PromptLoader` | section 级缓存的 Jinja2 装配器 |
| 52 | `.render(...)` | 渲染单个模板（cacheable+无变量→缓存） |
| 72 | `.assemble(...)` | 组装完整 prompt（override→静态→append 优先级） |

#### `engine/logging_filter.py` — 日志脱敏
| 行号 | 定义 | 描述 |
|------|------|------|
| 16 | `_SECRET_RULES` | 敏感模式（Bearer/sk-/Authorization/Cookie/JWT） |
| 50 | `RedactFilter` | 日志凭证脱敏过滤器 |

### backend/src/sdk/ — SDK 工具协议（4 个文件）
...
| 37 | `AskOption(BaseModel)` | 追问选项（label/description） |
| 43 | `AskQuestion(BaseModel)` | 单个追问问题（question/header/options/multi_select） |
| 51 | `AskSpec(BaseModel)` | 追问规格（questions 列表） |
| 57 | `ToolResult(BaseModel)` | 工具结果三层：artifact + reply/ask + extra/error_for_llm |
| 77 | `ClarificationRaised(Exception)` | 工具中途追问异常 |
...
| 行号 | 定义 | 描述 |
|------|------|------|
| 9 | `ToolRegistry` | register/all/get/describe_for_llm |

#### `sdk/asset_client.py` — AssetClient 抽象
| 行号 | 定义 | 描述 |
|------|------|------|
| 19 | `AssetClient(ABC)` | 资产来源抽象：get_template/list_templates/get_schema/get_guide/validate/persist/submit_data/query_data |

#### `sdk/sanitize.py` — Unicode 隐写清洗
| 行号 | 定义 | 描述 |
|------|------|------|
| 15 | `_DANGEROUS_RANGES` | 危险字符范围（零宽/方向反转/PUA） |
| 24 | `sanitize_text(text)` | 清洗：NFKC + 删 BOM + 删危险字符 |
| 42 | `sanitize_obj(obj)` | 递归清洗 dict/list/str |

### backend/src/llm/ — LLM 客户端

#### `llm/client.py` — 同步 OpenAI 兼容客户端
| 行号 | 定义 | 描述 |
|------|------|------|
| 29 | `LLMConfig(BaseModel)` | LLM 配置（base_url/api_key/model/temperature/max_tokens=200000/timeout） |
| 39 | `LLMClient` | 同步 OpenAI 兼容客户端 |
| 72 | `._log_call(...)` | 持久化 LLM 调用日志到 call_logs 表 |
| 99 | `.chat(...)` | chat completion（处理 Qwen3 reasoning_content 回退） |
| 178 | `.chat_json(...)` | chat + 解析 JSON：先 json_object 模式，失败回退纯文本提取 |
| 283 | `._parse_json_from_text(text)` | 从任意文本提取 JSON（纯/markdown/首尾大括号三级容错） |

### backend/src/services/ — 服务层

#### `services/upstream_client.py` — 上游 njmind-modeler HTTP 客户端
| 行号 | 定义 | 描述 |
|------|------|------|
| 34 | `_forward_headers` | threading.local（每请求透传 headers） |
| 64 | `UpstreamClient` | 上游 HTTP 客户端（httpx + 缓存 + 调用日志） |
| 144 | `.list_templates()` | GET /api/mcp/templates/list-templates |
| 172 | `.get_template(name)` | GET 模板（自动补 .json） |
| 285 | `.get_guide()` | GET guide.json |
| 321 | `.validate_form(...)` | POST /forms/validate，归一化 {pass}→{valid} |
| 399 | `.get_form(form_code)` | GET 已有表单配置 |
| 427 | `.create_form(...)` | POST /forms/create |
| 497 | `.health_check()` | 上游可达性检查 |

#### `services/conversation_store.py` — SQLite append-only 存储
| 行号 | 定义 | 描述 |
|------|------|------|
| 36 | `ConversationStore` | SQLite 存储（events + session_meta + call_logs 三表） |
| 119 | `.create_conversation(...)` | 创建会话 |
| 136 | `.list_conversations(...)` | 按用户列出会话 |
| 276 | `.add_message(...)` | 追加消息（append-only，role→kind 映射） |
| 311 | `.get_messages(conv_id)` | 从 events 重建 messages |
| 335 | `.save_call_log(...)` | 保存 LLM/上游调用日志 |
| 432 | `.append_event(...)` | 公开 append-only 追加事件 |
| 481 | `.load_pending_ask(conv_id)` | 加载最近一条 ask 事件 |

### backend/src/adapters/ — 适配器

#### `adapters/http_asset_client.py` — AssetClient HTTP 实现
| 行号 | 定义 | 描述 |
|------|------|------|
| 31 | `HttpAssetClient(AssetClient)` | 委托 UpstreamClient + sanitize 层 |
| 65 | `.validate_artifact(...)` | 归一化 {pass,errors:[str]}→{valid,errors:[{message}]} |
| 76 | `.persist_artifact(...)` | create/update 持久化 |
| 93 | `.submit_data(...)` | POST 通用数据提交（ASSET_BASE_URL） |
| 124 | `.query_data(...)` | GET 通用数据查询 |

### backend/src/api/ — FastAPI 路由

#### `api/config.py` — 配置/聊天路由
| 行号 | 定义 | 描述 |
|------|------|------|
| 31 | `ChatRequest(BaseModel)` | 统一聊天入口（message/conversation_id/answers/image_base64） |
| 69 | `_extract_forward_headers(...)` | 提取要透传给上游的 headers |
| 106 | `@router.post("/chat") chat(...)` | 统一聊天入口：stream_graph SSE |
| 201 | `@router.post("/validate") validate(...)` | 同步校验 FormConfig |

#### `api/conversations.py` — 会话 CRUD
| 行号 | 定义 | 描述 |
|------|------|------|
| 33 | `@router.post("") create_conversation` | 创建会话 |
| 45 | `@router.get("") list_conversations` | 列出（admin 看全部） |
| 59 | `@router.get("/{conv_id}") get_conversation` | 会话详情 |
| 78 | `@router.delete("/{conv_id}") delete_conversation` | 删除会话 |

#### `api/sse.py` — SSE 事件管理
| 行号 | 定义 | 描述 |
|------|------|------|
| 17 | `_STREAM_END` | 流结束哨兵 |
| 20 | `SSEEvent` | SSE 事件（event/data/to_sse） |
| 33 | `StreamManager` | 线程安全事件队列（loop 桥接） |
| 40 | `.stage(...)` | 推 stage 事件（线程安全，call_soon_threadsafe） |
| 73 | `async stream()` | 流式产出 SSE（120s keepalive） |

#### `api/skills.py` / `api/health.py`
| 行号 | 定义 | 描述 |
|------|------|------|
| skills:15 | `list_templates` | 列模板 |
| skills:45 | `get_guide` | 取 guide |
| health:14 | `health_check` | 健康检查（status/service/version/upstream） |
...
| 行号 | 定义 | 描述 |
|------|------|------|
| 46 | `lifespan(app)` | 启动：Store→Upstream→LLM→load_all_packs→build_graph→MCP |
| 101 | `app = FastAPI(...)` | 应用实例（version=0.4.0） |
| 108 | `CORSMiddleware` | CORS（allow_origins=["*"]） |
| 124 | `uvicorn.run(...)` | 入口（host 0.0.0.0:18080） |

#### `mcp_server.py` — MCP Server
| 行号 | 定义 | 描述 |
|------|------|------|
| 25 | `create_mcp_server(...)` | 创建 FastMCP server + 注册工具/资源 |
| 38 | `@mcp.tool() get_form_config` | 通过 graph.invoke 生成 FormConfig |
| 91 | `@mcp.tool() validate_form` | 校验表单 |

### backend/src/domains/ — Domain Packs

#### `domains/__init__.py` — Pack 自动发现
| 行号 | 定义 | 描述 |
|------|------|------|
| 27 | `discover_packs()` | 扫描 domains/ 下含 pack.py 的目录 |
| 89 | `load_all_packs()` | 加载所有 pack 并合并 registry |

#### `domains/njmind_form/pack.py`
| 行号 | 定义 | 描述 |
|------|------|------|
| 16 | `create_registry()` | 注册 6 个工具（create/modify/get/clone/image/chat） |

#### `domains/njmind_form/models.py`
| 行号 | 定义 | 描述 |
|------|------|------|
| 10 | `ParsedField(BaseModel)` | 解析出的字段（fieldTitleText/fieldTitleKey/formFieldType） |
| 20 | `FormConfig(BaseModel)` | njmind 表单配置（extra=allow） |
...
| 43 | `steps = [...]` | pipeline: fetch_guide/list_assets/parse_fields/fetch_templates/generate/validate |
| 113-254 | `_step_*` | 6 个 pipeline 步骤实现 |

#### `domains/njmind_form/tools/modify_form.py`
| 行号 | 定义 | 描述 |
|------|------|------|
| 22 | `ModifyFormTool(CompositeTool)` | 修改表单工具（requires_existing_artifact=True） |
| 37 | `steps = [...]` | pipeline: fetch_guide/modify/validate |

#### `domains/njmind_form/tools/chat.py` — 动态能力上报
| 行号 | 定义 | 描述 |
|------|------|------|
| 14 | `ChatTool(Tool)` | 闲聊工具（兜底） |
| 35 | `execute(...)` | 动态构建能力列表 + LLM 闲聊回复 |
| 89 | `_build_capabilities(ctx)` | 从 registry 动态生成能力列表 |

#### `domains/njmind_form/tools/get_form.py` / `clone_form.py` / `image_form.py`
| 行号 | 定义 | 描述 |
|------|------|------|
| get:14 | `GetFormTool(Tool)` | 查询表单（is_read_only=True） |
| clone:14 | `CloneFormTool(Tool)` | 克隆表单 |
| image:15 | `ImageFormTool(Tool)` | 图片识别表单（多模态） |

#### `domains/leave_application/` — Demo Pack
| 行号 | 定义 | 描述 |
|------|------|------|
| submit_leave:30 | `SubmitLeaveTool(CompositeTool)` | 提交请假（artifact_type=data，3 步管线） |
| query_status:17 | `QueryLeaveStatusTool(Tool)` | 查询请假状态（is_read_only=True） |
...
| 行号 | 定义 | 描述 |
|------|------|------|
| 6 | `useConversationStore` | 会话 store（setup 风格） |
| 18 | `pendingClarification` | 最后一条消息是否是追问 |
| 84 | `sendMessage(...)` | 统一发送：自动建会话→onResult 分流→追问恢复 |

#### `services/api.ts` — API 服务
| 行号 | 定义 | 描述 |
|------|------|------|
| 21 | `api.interceptors.request.use(...)` | 请求拦截：注入 X-User-Id + 透传 headers |
| 66 | `chat(...)` | 统一聊天入口（SSE 流） |
| 85 | `streamSSE(...)` | fetch + ReadableStream 解析 SSE |

#### `embed.ts` — 嵌入 SDK
| 行号 | 定义 | 描述 |
|------|------|------|
| 31 | `LLMFormModeler` | 嵌入 SDK 主类（iframe + postMessage） |
| 47 | `init()` | 创建浮动按钮 + 注册 message 监听 |
| 103 | `createIframe()` | 创建 iframe，postMessage MODELER_INIT |

#### `components/chat/ChatPanel.vue` / `ChatInput.vue`
| 行号 | 定义 | 描述 |
|------|------|------|
| ChatPanel:272 | `pipelineSteps` | 由 store.pipelineSteps 计算每步 status |
| ChatPanel:319 | `applyConfig(config)` | postMessage 配置到父系统 |
| ChatInput:95 | `onFileSelected(e)` | 选图：校验→转 base64 |
